...
with tab1:
    ...
    with col2:
        if 'Check_results' in st.session_state:
            ...
            st.text(f"Combined Score: {fraud_score:.2f}")
            col_ml, col_rule = st.columns(2)
            with col_ml: st.metric("ML Model Score", f"{ml_score:.2f}")
            with col_rule: st.metric("Rule-Based Score", f"{rule_score:.2f}")
...
with tab2:
    ...
    # Show current exact time
    st.info(f"🕒 Current Transaction Time: {full_datetime_display}")
    
    # Default location and device
    ...
    with col2:
        if 'transaction_results' in st.session_state:
            ...
            st.markdown(f"**Risk Level:** :{color}[{risk_level}]")
            st.progress(float(fraud_score))
            st.text(f"Fraud Score: {fraud_score:.2f}")
...
st.markdown("""
<style>
div[data-testid="column"]:has(button[key="dashboard_btn"]) {
